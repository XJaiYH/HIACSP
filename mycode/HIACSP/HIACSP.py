...
class HIACSP():
    ...
    def shrink(self, T):
        ...
        self.influence_norm = self.influence_norm / self.density #之前的结果用的是sqrt(density)，还没有跑只用density的，但结果应该差不多
        # tmp_norm = np.sort(self.influence_norm)
        threshold = np.mean(self.influence_norm)
        # threshold = tmp_norm[int(0.6 * self.Number)]

        G = np.mean(np.mean(self.knn_euclidean_dis[:, :self.K], axis=1), axis=0)
        self.over_influence_mean = []
...
if __name__=="__main__":
    ...
    parser.add_argument('--data_dir', type=str, default='../../data/', help='the director of the original dataset saved')
    ...
    parser.add_argument('--ratio', type=int, default=0, help='It is only used to verify the robustness of HIACSP\'s performance under'
                                                             'different overlap rate, the value of ratio can be selected in'
                                                             ' [10, 20, 30, 40, 50], datasets are R15, Gauss17, Two, and A1')
    parser.add_argument('--load_data_local', action="store_false", help='Default value is True. If use local dataset, the value should be set as True, otherwise setting as False')
    parser.add_argument('--data_id', type=int, default=78, help='If you want to download dataset from uci, please set the data_id of the dataset you want')
    parser.add_argument('--with_label', action="store_false", help='Default value is True. If the dataset contains labels in the last column, set this value as True. Otherwise, '
                                                                  'this software will load labels from local file which name is '
                                                                  '"args.data_dir/args.data_name/args.data_name-label.txt"')
    parser.add_argument('--visualization', action="store_true", help='Whether to visualization the intermediate results, default value is False')
    ...
    parser.add_argument('--verbose', action="store_true", help='Whether to print the process information, default value is False')
    parser.add_argument('--p', type=int, default=4, help='hyper-parameter p')
    prompt = '【System Information: 】'
    args = parser.parse_args()

    algo_name = 'HIACSP-p' + str(args.p)
    data, label = load_data(args)
    print(args.with_label)
    ...
    # plotCluster(hiacsp.ori_data, hiacsp.label, args.data_name, args)
    total_time += time.time() - start

    with tqdm(total=args.D) as tbar:
        for i in range(args.D):
            start = time.time()
            res = hiacsp.shrink(args.T)
            if args.visualization:
                plotCluster(res, hiacsp.label, '(B) Ameliorated distribution', args, [])
            total_time += time.time() - start
