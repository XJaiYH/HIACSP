# cg means conventional gravitation
import argparse
import heapq
import time
import sys
...
from mycode.util.visualization import plotCluster
import warnings
...
class HIACSP():
    ...
    def shrink(self, T):
        ...
        self.influence_norm = self.influence_norm / np.sqrt(self.density)
        threshold = np.mean(self.influence_norm)

        G = np.mean(self.knn_euclidean_dis[:, 0])
        self.over_influence_mean = []
        ...
        for i in range(self.Number):
            ...
            while j < self.K and self.knn[i, j] != -1:
                ...
                dis_1 = np.sqrt(np.sum(np.square(self.res_data[self.knn[i, 0]] - self.res_data[i])))
                if dis_2 == 0:
                    j += 1
                    continue
                ff = (self.res_data[self.knn[i, j]] - self.res_data[i])
                fff = (dis_1 / dis_2)
                displacement += ff * fff * G
                j += 1
        ...
        for kk in k_set:
            for tt in T_set:
                ...
                for i in range(args.D):
                    ...
                    if args.visualization:
                        plotCluster(res, hiacsp.label, 'test', args, [])
